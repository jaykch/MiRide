...
/*
 * Class:		Menu
 * Description:	This class represents the user interface menu that handles 
* 				input validation and connects to system.
...
public class Menu
{
			...
			do
			{
				...
				} else
				{
					...
					case "SS":
						...
						System.out.println(application.displaySpecificCar(console.nextLine().toUpperCase()));
						break;
					case "SD":
		...
		try
		{
			...
			if (id.length() != 0)
			{
				...
				if (!result)
				{
					...
					{
						String carRegistrationNumber = application.createCar(id, make, model, driverName, numPassengers,
								serviceType, standardFee, listOfRefreshments.split(","));
						System.out.println(carRegistrationNumber);
					}
				} else
				{
	...
	/*
	 * Search for All Available Cars in Application. Searching is done on basis of
	 * date which checks if car is available on the specific date and the type of
	 * car.
	 */
	private boolean SearchForAvailableCars()
	{
		try
		{
			...
			String typeOfCar = console.nextLine().toUpperCase();
			...
			{
				System.out.println("The followig cars are available: \n");
				for (int i = 0; i < availableCars.length; i++)
				{
					System.out.println(availableCars[i]);
				}
