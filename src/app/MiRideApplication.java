 ...
 * Description:		This class represents the system that manages the 
 *              	collection of data. 
...
public class MiRideApplication
{
	...
	public String createCar(String id, String make, String model, String driverName, int numPassengers,
			String serviceType, double Fee, String[] refreshments)
	{
	...
	/*
	 * Get Booking by using car registration number
	 */
	...
	public boolean seedData()
	{
		 ...
		 */
		String[] refreshments = { "Pepsi", "Chocolates", "Walkers" };
	...
	/*
	 * Checks If Car is present in system using registration number
	 */
	...
	private Car getCarById(String regNo)
	{
		...
		* 		IF array index is not empty
		* 			IF car registration number is equal to registration number
		* 				return that car
		* 		else return null
	...
	public Car[] sortInDescendingOrder(String carType)
	{
		...
		while (!sorted)
		{
			...
			for (int i = 0; i < array.length - 1; i++)
			{
				...
				if ((currentCarFirstPart.compareTo(nextCarFirstPart) == 0)
						&& (currentCarSecondPart < nextCarSecondPart))
				{
					temp = array[i];
					array[i] = array[i + 1];
					array[i + 1] = temp;
	...
	/*
	 * Parses the record stored in the data file/backup data file so that it can be
	...
	public void parseRecord(String record)
	{
		...
		} else
		{
			if (record.contains(currentBookingsDelimiter))
			{
				...
				currentBookings = record
						.substring(startIndexOfCurrentBookingDelimiter + currentBookingsDelimiter.length(),
								endIndexOfCurrentBookingDelimiter)
						.split("\\|");
			...
			if (record.contains(pastBookingsDelimiter))
			{
				...
				pastBookings = record.substring(startIndexOfPastBookingDelimiter + currentBookingsDelimiter.length(),
						endIndexOfPastBookingDelimiter).split("\\|");
			}
		}
	...
	public void loadCar(String record)
	{
		...
		} else
		{
			...
			createCar(temp[1], temp[2], temp[3], temp[4], Integer.parseInt(temp[5]), temp[0],
					Double.parseDouble(temp[7]), refreshments);
		}
	}
	...
	public void loadCurrentBookings(String record, String[] currentBookings)
	{
		...
		for (int i = 0; i < temp_all_current_bookings.length; i++)
		{
			...
			if (temp_record[0].equals("SD"))
			{
				...
				sdCar.autoBook(name_of_booker[0], name_of_booker[0], new DateTime(day, month, year),
						Integer.parseInt(temp_current_booking[4]));
			} else
			{
				SilverServiceCar ssCar = (SilverServiceCar) getCarById(temp_record[1]);
				ssCar.autoBook(name_of_booker[0], name_of_booker[0], new DateTime(day, month, year),
						Integer.parseInt(temp_current_booking[4]));
			}
		}
	}
	...
	public void loadPastBookings(String record, String[] pastBookings)
	{
		...
		for (int i = 0; i < temp_all_past_bookings.length; i++)
		{
			...
			{
				Car sdCar = getCarById(temp_record[1]);
				Booking booking = new Booking(name_of_booker[0], name_of_booker[0], new DateTime(day, month, year),
						Integer.parseInt(temp_past_booking[4]), sdCar, false);
				sdCar.autoCompleteBooking(booking, Double.parseDouble(temp_past_booking[5]),
						Double.parseDouble(temp_past_booking[6]), sdCar.getSTANDARD_BOOKING_FEE());

			} else
			{
				SilverServiceCar ssCar = (SilverServiceCar) getCarById(temp_record[1]);
				Booking booking = new Booking(name_of_booker[0], name_of_booker[0], new DateTime(day, month, year),
						Integer.parseInt(temp_past_booking[4]), ssCar, false);
				ssCar.autoCompleteBooking(booking, Double.parseDouble(temp_past_booking[5]),
						Double.parseDouble(temp_past_booking[6]), ssCar.getBookingFee());
	...
	/*
	 * A helper function to clean the booking arrays when parsed
	 */
